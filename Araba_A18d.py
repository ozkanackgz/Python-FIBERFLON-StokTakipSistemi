...
from Araba_A18 import *
import sqlite3


uygulama = QApplication(sys.argv)
pencere = QMainWindow()
ui = Ui_arabalarA18_ekran()
ui.setupUi(pencere)
pencere.show()
...
def kayit_listele():
    ui.A18_tw.clear()
    ui.A18_tw.setHorizontalHeaderLabels(("Lot", "Kalite", "Genişlik"))
    ui.A18_tw.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
...
def kayit_sil():
    ...
    if sil_mesaj == QMessageBox.Yes:
        secilen_satir = ui.A18_tw.currentRow()  
        if secilen_satir == -1:
            ui.statusbar.showMessage("Silinecek bir satır seçilmedi!")
            return

      
        silinecek_lot = ui.A18_tw.item(secilen_satir, 0).text()
        ...
        try:
            ...
            islem.execute(sorgu_a18, (silinecek_lot,))
...
ui.listele18_btn.clicked.connect(kayit_listele)
ui.sil18_btn.clicked.connect(kayit_sil)
